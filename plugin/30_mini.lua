...
now(function() vim.cmd('colorscheme teide') end)
...
later(function()
  ...
  -- Safe exit from just about everything.
  -- Use `jk` or `kj` in rapid combo, will delete what was added then bail out.
  ...
  -- Show notification if there is too much movement by repeating same keys.
  ...
  MiniKeymap.map_combo('i', 'kk', '<BS><BS><Esc>[s1z=gi<Right>', {desc = 'Fix previous spelling mistake'})

  -- Get rid of search highlights.
  MiniKeymap.map_combo({ 'n', 'i', 'x', 'c' }, '<Esc><Esc>', function() vim.cmd('nohlsearch') end, {desc = 'Clear Search Highlighting'})

  -- Buffer navigation.
  MiniKeymap.map_combo({ 'n', 'x' }, 'll', 'g$', {desc = 'jump to start of line'})
  MiniKeymap.map_combo({ 'n', 'x' }, 'hh', 'g^', {desc = 'jump to end of line'})
  MiniKeymap.map_combo({ 'n', 'x' }, 'jj', '}', {desc = 'jump to next blank line'})
  MiniKeymap.map_combo({ 'n', 'x' }, 'kk', '{', {desc = 'jump to prev blank line'})
end)
